/**
 * @component command · stock: shadcn base-luma
 * @regen     pnpm ui:add command
 * @frost     surface — Command container uses the `frosted` material
 * @delta     ~ Command root: bg-popover → frosted (compounds when inside a frosted dialog/popover)
 */
"use client";
...
function Command({
  ...
  return (
    <CommandPrimitive
      className={cn(
        "frosted flex size-full flex-col overflow-hidden rounded-4xl p-1 text-popover-foreground",
        className
      )}
      data-slot="command"
      {...props}
