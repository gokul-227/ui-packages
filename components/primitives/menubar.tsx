/**
 * @component menubar · stock: shadcn base-luma (menuColor: inverted-translucent)
 * @regen     pnpm ui:add menubar
 * @frost     surface — menu content + sub-content panels use the `frosted` material
 * @delta     ~ content panels: bg-popover/70 + ::before backdrop-blur-2xl/saturate-150 → frosted
 *            - drop the forced `dark` inversion + the **:data-[variant=destructive] accent overrides
...
function MenubarContent({
  ...
  return (
    <DropdownMenuContent
      ...
      className={cn(
        "data-[side=bottom]:slide-in-from-top-2 data-[side=inline-end]:slide-in-from-left-2 data-[side=inline-start]:slide-in-from-right-2 data-[side=left]:slide-in-from-right-2 data-[side=right]:slide-in-from-left-2 data-[side=top]:slide-in-from-bottom-2 data-open:fade-in-0 data-open:zoom-in-95 frosted min-w-48 animate-none! rounded-3xl p-1.5 text-popover-foreground ring-1 ring-foreground/5 duration-100 data-open:animate-in **:data-[slot$=-item]:data-highlighted:bg-foreground/10 **:data-[slot$=-separator]:bg-foreground/5 **:data-[slot$=-trigger]:aria-expanded:bg-foreground/10! **:data-[slot$=-item]:focus:bg-foreground/10 **:data-[slot$=-trigger]:focus:bg-foreground/10 dark:ring-foreground/10",
        className
      )}
