...
import { Separator } from "@aec-craft/ui/components/primitives/separator";
import {
  Sheet,
  SheetContent,
  SheetDescription,
  SheetHeader,
  SheetTitle,
} from "@aec-craft/ui/components/primitives/sheet";
...
import { useIsMobile } from "@aec-craft/ui/hooks/use-mobile";
...
/**
 * DELTA vs stock shadcn: returns a detached default instead of throwing when
 * there is no `SidebarProvider`.
 *
 * `SidebarMenuButton` calls this unconditionally but only reads it to place a
 * collapsed-state tooltip, so throwing made the one genuinely reusable part of
 * this file unusable outside an app shell — and the settings surface answered
 * by rebuilding the button, which then drifted off the `sidebar-*` tokens. The
 * default reads as "expanded, not mobile", which is exactly what a menu button
 * outside a collapsible shell is.
 */
function useSidebar(): SidebarContextProps {
  return React.useContext(SidebarContext) ?? DETACHED_SIDEBAR;
}

const DETACHED_SIDEBAR: SidebarContextProps = Object.freeze({
  state: "expanded",
  open: true,
  setOpen: () => undefined,
  openMobile: false,
