/**
 * @component card · stock: shadcn base-luma
 * @regen     pnpm ui:add card
 * @frost     surface — Card panel uses the `frosted` material
 * @delta     ~ Card: bg-card → frosted (translucent fill + backdrop blur; ring gives the edge)
 *            - drop shadow-md (flat frosted look)
 */
...
function Card({
  ...
  return (
    <div
      className={cn(
        "group/card frosted flex flex-col gap-(--card-spacing) overflow-hidden rounded-4xl py-(--card-spacing) text-card-foreground text-sm ring-1 ring-foreground/5 [--card-spacing:--spacing(6)] has-[>img:first-child]:pt-0 dark:ring-foreground/10 data-[size=sm]:[--card-spacing:--spacing(4)] *:[img:first-child]:rounded-t-4xl *:[img:last-child]:rounded-b-4xl",
        className
      )}
      data-size={size}
      data-slot="card"
