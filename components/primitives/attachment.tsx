/**
 * @component attachment · stock: shadcn base-luma
 * @regen     pnpm ui:add attachment
 * @frost     surface — Attachment card uses the `frosted` material (matches `card`)
 * @delta     ~ Attachment: bg-card → frosted (translucent fill + backdrop blur)
 *            ~ hover tint: bg-muted/50 → color-mix over --frost-fill, so hover deepens the
 *              frost instead of replacing it and exposing the saturated backdrop
 */
...
const attachmentVariants = cva(
  "group/attachment frosted relative flex w-fit min-w-0 max-w-full shrink-0 flex-wrap rounded-3xl border text-card-foreground transition-colors focus-within:ring-1 focus-within:ring-ring/30 has-[>a,>button]:hover:bg-[color-mix(in_oklab,var(--foreground)_6%,var(--frost-fill))] data-[state=error]:border-destructive/30 data-[state=idle]:border-dashed",
  {
    variants: {
      size: {
        default:
